...
def get_edges(n1, n2, n):
    ...
    unique_valid_nodes, reverse_index = np.unique(all_valid_nodes, return_inverse=True)
    return unique_valid_nodes, reverse_index


def extract_graphs_from_unique_nodes(
    ...
    for as_name_file in as_names:
        ...
        try:
            ...
            file_logger.warning(
                "AS {} does not have any link.".format(as_name_file.split(".")[0])
            )
            continue
        with open(as_nodes_path, "r") as f:
            ...
            for node_group in f:
                ...
                node_group = np.array(list(map(lambda s: int(s[1:]), node_group)))
                ...
                edges = np.stack(
                    (
                        flat_edges[0 : len(flat_edges) // 2],
                        flat_edges[len(flat_edges) // 2 :],
                    )
                )
                graph_path = os.path.join(assets_path, "{}-{}".format(as_name, k))
...
def extract_graphs_from_joining_nodes(
    ...
    for as_name in ases:
        ...
        links_non_loops = links_df.loc[links_df["label1"] != links_df["label2"]]
        ...
        file_logger.info(
            "For AS {},  {} nodes was used of {}, and {} links was used of {}.".format(
        ...
        file_logger.info(
            ...
            "{} ASes with number of nodes with different positions < 20".format(
                num_of_small_unique_nodes_ases, num_of_empty_ases
            )
        )


def create_graphs_from_ases(
    geo_path, links_path, unique_nodes_path=None, add_noise=True
):
